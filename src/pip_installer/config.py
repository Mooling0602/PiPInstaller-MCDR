...
def _get_fixed_aliases_data(data: dict) -> dict:
    """用于删除错误的命令别名配置，接受一个命令别名的字典数据并返回剔除错误键值对后的正确数据，随时可能弃用。"""
    keys_to_remove: list = []
...
def register_command_aliases(server: PluginServerInterface):
    if os.path.exists(command_aliases_config):
        server.logger.info("检测到Command Aliases配置存在，正在尝试自动添加命令别名……")
        ...
        server.logger.info("Command Aliases插件配置文件不存在，无法注册命令别名。")
        return
    command_aliases_data: dict = _get_fixed_aliases_data(copy.deepcopy(existing_data))
    ...
    server.logger.info("成功添加或更新命令别名！使用 !!pip usage 查看详细用法。")